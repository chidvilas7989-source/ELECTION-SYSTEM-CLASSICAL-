...
# Classical cryptography compatibility wrapper
def get_classical_key_compatible(length=32):
    ...
    try:
        ...
        # Your classical function returns bytes, ensure we have the right format for crypto_utils
        if isinstance(key_bytes, bytes):
            return key_bytes
        else:
            # Fallback if key is returned as string/other format
...
def cast_vote():
    ...
    try:
        # Use the classical cryptography compatibility wrapper
...
def reset_system():
    if request.json.get("password") != "admin@123":
...
if __name__=="__main__":
    print("🚀 Classical Cryptographic Voting API running on :5000")
    print("🔐 Using AES-256 + RSA-2048 instead of quantum cryptography")
    print("✅ Quantum dependencies removed - system ready")
